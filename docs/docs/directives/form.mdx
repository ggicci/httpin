...
**form** is a [directive](/advanced/concepts#directives) who decodes a field from HTTP form data, i.e. [`http.Request.Form`](https://pkg.go.dev/net/http#Request).
...
**form** will examine values of the keys one by one (KEY1 -> KEY2 -> ...), the **first non-empty** value will be used to set the corresponding field of the input struct.

:::caution

In most cases, the `form` directive just works like [`query`](/directives/query). Because it also visit data from the URL querystring.

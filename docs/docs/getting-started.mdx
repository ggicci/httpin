...
- [Headers](/directives/header), e.g. `Authorization: Bearer xxx`
...
and [more](/directives/custom) of an HTTP request into a struct in Go. And vice versa, we support encoding a struct into an HTTP request.

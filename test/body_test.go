...
func TestBodyDecoder_JSON(t *testing.T) {
	assert := assert.New(t)
	core, err := httpin.New(JSONBodyPayloadWithBodyDirective{})
	assert.NoError(err)
...
func TestBodyDecoder_XML(t *testing.T) {
	...
	assert := assert.New(t)
	core, err := httpin.New(XMLBodyPayloadWithBodyDirective{})
	assert.NoError(err)
...
func TestBodyDecoder_ErrUnknownBodyFormat(t *testing.T) {
	...
	core, err := httpin.New(UnknownBodyFormatPayload{})
...
func TestRegisterBody(t *testing.T) {
	assert.NotPanics(t, func() {
		directive.RegisterBodyFormat("yaml", &yamlBody{})
	})
	assert.Panics(t, func() {
		directive.RegisterBodyFormat("yaml", &yamlBody{})
	})

	core, err := httpin.New(YamlInput{})
	assert.NoError(t, err)
...
func TestRegisterBody_nil(t *testing.T) {
	assert.Panics(t, func() {
		directive.RegisterBodyFormat("toml", nil)
	})
}

func TestRegisterBody_forceReplace(t *testing.T) {
	assert.NotPanics(t, func() {
		directive.RegisterBodyFormat("yaml", &yamlBody{}, true)
	})
	assert.NotPanics(t, func() {
		directive.RegisterBodyFormat("yaml", &yamlBody{}, true)
	})
...
func TestRegisterBody_forceReplace_withEmptyBodyFormat(t *testing.T) {
	assert.PanicsWithError(t, "httpin: body format cannot be empty", func() {
		directive.RegisterBodyFormat("", &yamlBody{}, true)
	})
}

...
type UpdateGitHubIssueInput struct {
	...
	Attachments []*httpin.File `in:"form=attachment"`
}

func TestRegisterFileType(t *testing.T) {
	assert.PanicsWithError(t, "httpin: duplicate file type: *internal.File", func() {
		directive.RegisterFileType[*httpin.File](nil)
	})
	assert.PanicsWithError(t, "httpin: file decoder cannot be nil", func() {
		directive.RegisterFileType[*BadFile](nil)
	})
}

func TestFile_OpenUploadStream_FailOnInvalidUpload(t *testing.T) {
	file := &httpin.File{} // invalid upload, must use UploadWithFilename or UploadWithReader
...
func TestMultipartForm_UploadSingleFile(t *testing.T) {
	...
	core, err := httpin.New(UpdateUserProfileInput{})
	assert.NoError(err)
...
func TestMultipartForm_UploadSingleFile_FailOnDecodeError(t *testing.T) {
	directive.RegisterFileType[*BadFile](badFileDecoder{})
	...
	core, err := httpin.New(AccountUpdate{})
	assert.NoError(err)
...
func TestMultipartForm_UploadMultiFiles(t *testing.T) {
	...
	core, err := httpin.New(UpdateGitHubIssueInput{})
	assert.NoError(err)
	gotInput, err := core.Decode(r)
	assert.NoError(err)
...
func TestMultipartFormEncode_UploadFilename(t *testing.T) {
	type Post struct {
		Username string         `in:"form=username"`
		Main     *httpin.File   `in:"form=main"`
		Pictures []*httpin.File `in:"form=pictures"`
	}
	...
	payload := &Post{
		Username: "ggicci",
		Main:     httpin.UploadWithFilename(mainFilename),
		Pictures: []*httpin.File{
			httpin.UploadWithFilename(pic1Filename),
			httpin.UploadWithFilename(pic2Filename),
		},
	}
	core, err := httpin.New(Post{})
...
func TestMultipartFormEncode_UploadReader(t *testing.T) {
	...
	payload := &Post{
		Username: "ggicci",
		Main:     httpin.UploadWithReader(io.NopCloser(mainReader)),
		Pictures: []*httpin.File{
			httpin.UploadWithReader(io.NopCloser(pic1Reader)),
			httpin.UploadWithReader(io.NopCloser(pic2Reader)),
		},
	}
	core, err := httpin.New(Post{})
	assert.NoError(t, err)
...
func assertDecodedFile(t *testing.T, gotFile *httpin.File, filename string, content []byte) {
